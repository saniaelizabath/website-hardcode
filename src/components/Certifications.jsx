  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Certifications Grid - Mobile: single column, Laptop: 4 columns */}
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6 mb-6 sm:mb-10 lg:mb-12">
          {certifications.map((cert, index) => (
            <div
              key={index}
              className={`group relative bg-white/75 backdrop-blur-md rounded-xl sm:rounded-2xl border border-blue-300/40 hover:border-blue-500/70 transition-all duration-500 overflow-hidden hover:shadow-2xl hover:shadow-blue-300/50 transform hover:-translate-y-2 ${
            ...
            >
              {/* Content */}
              <div className="relative p-4 sm:p-5 lg:p-6 flex flex-col items-center text-center h-full">
                {/* Image Container - Compressed on mobile */}
                <div className="w-full h-28 sm:h-36 lg:h-40 mb-2.5 sm:mb-4 flex items-center justify-center bg-slate-50/80 rounded-lg sm:rounded-xl p-2.5 sm:p-4">
                  <img
                    src={cert.image}
                    alt={cert.title}
                    className="max-w-full max-h-full object-contain group-hover:scale-110 transition-transform duration-300"
